...
class Xruner:
    def __init__(self, naca=None, airfoil=None, Re=None, mach=None, pan=250, ite=400, verbose=True):
        ...
        self.airfoil = airfoil  #Path to 'txt' or 'dat' airfoil coordinates file
        self.Re = Re            #
        self.pan = pan
        self.ite = ite
        self.mach = mach
    ...
    QUIT
    """
        execute = run(["xfoil.exe"], input=script, text=True, capture_output=True, timeout=3)
        if self.verbose:
    ...
    def run_alpha(self, AOA, filename, dumpfile=None, airfoil_plot=False):
        ...
        if bool(dumpfile):
            ...
            return self.save_polar(filename, bool(self.Re)), dumps
        return self.save_polar(filename, bool(self.Re))
    ...
    def run_aseq(self, aseq, filename, plot_data=[]):
        ...
        if bool(plot_data):
            polars = self.save_polar(filename, bool(self.Re))
